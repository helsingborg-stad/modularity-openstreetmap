...
class ComponentsJs
{
    private $components = [];
    private $lang = [];
    private $viewPath   = MODULARITYOPENSTREETMAP_VIEW_PATH . 'js/object/';
    ...
    private function renderView(string $view, array $data = [], bool $compress = true): string
    {
        if (function_exists('open_street_map_render_blade_view')) {
            return open_street_map_render_blade_view(
        ...
        throw new \RuntimeException('open_street_map_render_blade_view() does not exist');
    }
    ...
    private function sanitizeViewPath(string $view): string
    {
        return str_replace(
            ".blade.php",
            "",
            str_replace(MODULARITYOPENSTREETMAP_VIEW_PATH, "", $view)
        );
    }
}